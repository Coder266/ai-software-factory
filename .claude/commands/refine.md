...
goes through `/set-status`**, not by hand-editing frontmatter; even when you later unblock a
`new` story to `ready`, make that transition with `/set-status <EXP-id> ready` so the
frontmatter and the `## Status` block stay in sync and the change is committed to `main`.
